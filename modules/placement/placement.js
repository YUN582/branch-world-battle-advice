...
function setSubTool(toolId) {
  ...
  // 이전 도구가 image였으면 스탬프 미리보기 정리
  if (_state.currentTool === 'image') {
    _state.lastStampSize = null;
  }
  // 스탬프 미리보기 DOM 정리 (이전 도구의 잔상 방지)
  if (_preview) {
    _preview.classList.remove('bwbr-placement-preview--visible');
    _preview.innerHTML = '';
  }
...
function _redrawShapeMergePreview() {
  ...
  for (var i = 0; i < _shapeMergeBuffer.length; i++) {
    ...
    if (angle) {
      ...
      if (Math.floor(aabb.minX) < minX) minX = Math.floor(aabb.minX);
      if (Math.floor(aabb.minY) < minY) minY = Math.floor(aabb.minY);
      if (Math.ceil(aabb.maxX) > maxX) maxX = Math.ceil(aabb.maxX);
      if (Math.ceil(aabb.maxY) > maxY) maxY = Math.ceil(aabb.maxY);
    } else {
      if (mc.x < minX) minX = mc.x;
      if (mc.y < minY) minY = mc.y;
      if (mc.x + mc.width > maxX) maxX = mc.x + mc.width;
      if (mc.y + mc.height > maxY) maxY = mc.y + mc.height;
    }
  }
  var tileW = maxX - minX;
  var tileH = maxY - minY;
  ...
  for (var i = 0; i < _shapeMergeBuffer.length; i++) {
    ...
    entries.push({
      ...
      relY: (mc.y - minY) * scale + inset,
      drawW: mc.width * scale - inset * 2,
      drawH: mc.height * scale - inset * 2,
...
// - 결과: 개별 배치와 동일한 stroke 두께 + 내부 경계 stroke만 제거
...
function _finishShapeMerge() {
  ...
  var tileW = pureMaxX - pureMinX;
  var tileH = pureMaxY - pureMinY;
  if (tileW <= 0 || tileH <= 0) return;

  // 2. 캔버스: 정확한 타일 크기 (패딩 없음 — 인셋으로 stroke/jitter 공간 확보)
  var cw = tileW * scale;
  var ch = tileH * scale;
  ...
  for (var i = 0; i < _shapeMergeBuffer.length; i++) {
    ...
    entries.push({
      ...
      relY: (mc.y - pureMinY) * scale + inset,
      drawW: mc.width * scale - inset * 2,
      drawH: mc.height * scale - inset * 2,
      scaledSettings: scaledSettings,
      angle: entry.angle || 0
  ...
  // 6. 스테이징: unpadded mapCoords (개별 배치와 동일한 압축 비율)
  readSettingsFromDOM();
  var obj = {
    id: Date.now() + '-' + Math.random().toString(36).slice(2, 8),
...
function _renderShapeToCtx(ctx, type, x, y, w, h, settings) {
  ...
  if (settings.strokeSize > 0 && settings.strokeOpacity > 0) {
    ...
    ctx.strokeStyle = settings.strokeColor;
    ctx.lineWidth = settings.strokeSize;
    ctx.lineJoin = (settings.cornerRadius && settings.cornerRadius > 0) ? 'round' : 'miter';
    ctx.stroke();
  }
  ctx.restore();
}
...
function _renderShapeDataUrl(mapCoords) {
  ...
  // 캔버스: 정확한 타일 크기 (패딩 없음 — 인셋으로 stroke/jitter 공간 확보)
  ...
  canvas.height = ch;
  var ctx = canvas.getContext('2d');
  _renderShapeToCtx(ctx, _shapeSettings.shapeType, inset, inset, cw - inset * 2, ch - inset * 2, scaledSettings);
...
function createConfirmBar() {
  ...
  cancelBtn.addEventListener('click', function() {
    if (_shapeMergeMode && _shapeMergeBuffer.length > 0) {
      _clearShapeMergeBuffer();
    } else {
      clearAllStaged();
    }
  });
  _confirmBar._cancelBtn = cancelBtn;
  ...
  _confirmBtnEl.addEventListener('click', function() {
    if (_shapeMergeMode && _shapeMergeBuffer.length > 0) {
      _finishShapeMerge();
    } else {
      showConfirmDialog();
...
function updateConfirmBar() {
  ...
  var stagedCount = _state.stagedObjects.length;
  var mergeCount = _shapeMergeMode ? _shapeMergeBuffer.length : 0;
  _confirmBar.classList.add('bwbr-place-confirm-bar--visible');

  // 카운트 표시: 연속 모드에서 버퍼가 있으면 우선 표시
  if (mergeCount > 0) {
    _stagedCountEl.textContent = '연속: ' + mergeCount + '개';
  } else {
    _stagedCountEl.textContent = stagedCount > 0 ? stagedCount + '개 배치됨' : '배치 없음';
  }

  // 버튼 상태
  if (_confirmBtnEl) {
    if (mergeCount > 0) {
      _confirmBtnEl.textContent = '✓ 병합 완료';
      _confirmBtnEl.disabled = false;
    } else {
      _confirmBtnEl.textContent = '✓ 확인';
      _confirmBtnEl.disabled = stagedCount === 0;
    }
  }
  if (_confirmBar._cancelBtn) {
    _confirmBar._cancelBtn.textContent = mergeCount > 0 ? '🗑 모두 지우기' : '전체 취소';
  }
