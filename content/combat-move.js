...
(function () {
  ...
  function enableCombatMode() {
    if (_combatMode) return;
    // 가지세계 모듈 비활성화 시 차단
    if (!_isCombatModuleEnabled()) {
      if (window.BWBR_FabButtons) {
        window.BWBR_FabButtons.showToast('가지세계 모듈이 비활성화되어 있습니다', { bg: 'rgba(211,47,47,0.92)', color: '#fff', duration: 2500 });
      }
      return;
    }
    // 배치 모드 활성화 중이면 차단
  ...
  function _isCombatModuleEnabled() {
    // 모듈 로더가 아직 로드되지 않았으면 활성으로 간주 (초기화 시점)
    if (!window.BWBR_ModuleLoader || !window.BWBR_ModuleLoader.isLoaded()) return true;
    return window.BWBR_ModuleLoader.isEnabled('branch-world');
  }
  ...
  function setupFabButton() {
    ...
    // 모듈 로딩 완료 후 비활성이면 버튼 제거
    document.addEventListener('bwbr-modules-loaded', function () {
      if (!window.BWBR_ModuleLoader.isEnabled('branch-world') && window.BWBR_FabButtons) {
        window.BWBR_FabButtons.unregister('combat-mode');
      }
    }, { once: true });
  }
  ...
  document.addEventListener('keydown', function(e) {
    ...
    if (e.altKey && (e.key === '*' || e.code === 'NumpadMultiply' || (e.shiftKey && e.code === 'Digit8'))) {
      if (!_isCombatModuleEnabled()) return;
      e.preventDefault();
      e.stopPropagation();
      toggleCombatMode();
    }
